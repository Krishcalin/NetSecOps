...
| [Architecture at a glance](#architecture-at-a-glance) | Five diagrams: the components, the read-only guard, the pipeline, path analysis, deployment |
...
Five pictures, in the order they answer the questions people actually ask. All of them
are generated by [one script](docs/diagrams/gen_diagrams.py) with no dependency beyond
Pillow — see [`docs/diagrams/`](docs/diagrams/) to regenerate them.

### What the pieces are

![Logical architecture](docs/diagrams/fig1_architecture.png)

*A React console behind Caddy, a FastAPI control plane, PostgreSQL, and a pool of
workers. The services layer holds the business logic and knows nothing about HTTP, which
is why the same code serves a request and runs a scheduled job. Workers are the only
thing in the system that opens a connection to a device.*

### How the read-only guarantee is enforced

![The read-only guarantee](docs/diagrams/fig2_readonly.png)

*The four layers described above, drawn in the order a command meets them. Every one of
them acts **before transmission** — nothing is filtered after the fact, and there is no
path around them, because an adapter holds a guarded session rather than a transport.*

### How a device becomes a finding

![From a device to a finding](docs/diagrams/fig3_pipeline.png)

*Collection, a sealed artefact, one parser per platform, and then the normalised
configuration model that every engine reads. Nothing downstream of that model parses
vendor syntax, which is what lets a check be written once and evaluated against thirteen
platforms.*

### Why a path answer has two verdicts

![A path answer has two axes](docs/diagrams/fig4_path.png)
...
![What actually runs](docs/diagrams/fig5_topology.png)

*One Docker Compose stack on your own infrastructure, with outbound connections only to
the devices you name, only on tcp/22 and tcp/443, and only read-only. A hundred-device
install and a two-thousand-device install run the same product.*
