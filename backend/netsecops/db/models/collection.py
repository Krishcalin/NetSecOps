...
class Collection(Base, UUIDPrimaryKeyMixin, OrgMixin, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_collections_device_created", "device_id", "created_at"),
        # Declared here as well as in migration 0015, which created it and is the only
        # place it existed. `alembic check` compares the models against the database
        # and reported it as an index to *drop* — so the next autogenerated migration
        # would have removed it, and the nightly retention scan would have gone back to
        # a sequential scan of the fastest-growing table in the schema with nothing
        # failing to say so.
        Index(
